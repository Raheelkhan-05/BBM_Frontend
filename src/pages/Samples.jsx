...
import {
  ...
  ChevronRight, Clock, Inbox, Tag as TagIcon, Layers
} from "lucide-react";
...
export default function Samples() {
  ...
  const [form, setForm]             = useState({ sample_status: "", follow_up_date: "" });
  ...
  async function openModal(sample) {
    setActive(sample);
    setForm({ sample_status: sample.sample_status || "", follow_up_date: sample.follow_up_date || "" });
    setFormError(""); setShowModal(true);
    setLL(true); setLogs([]);
    try {
      const res  = await fetch(`${API}/api/samples/${sample.id}/logs`, { headers });
      const data = await res.json();
  ...
  async function handleUpdate(e) {
    e.preventDefault();
    if (!form.sample_status) { setFormError("Please select a status"); return; }
    setSaving(true); setFormError("");
    try {
      const res  = await fetch(`${API}/api/samples/${active.id}`, {
      ...
      <div className="mx-auto max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8">
        ...
        <div className="mb-7 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl">Samples</h1>
          </div>
        ...
        {!loading && !error && filtered.length === 0 && (
          <EmptyState
            ...
            subtitle={hasActiveFilters ? "Try adjusting your filters" : "Samples linked to RFQs will appear here"}
          />
        )}
          ...
          <motion.div layout className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <AnimatePresence mode="popLayout">
              {filtered.map((s, i) => {
                const rfq  = s.rfqs   || {};
                const lead = rfq.leads || {};
                const tone = statusTone[s.sample_status] || defaultTone;
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <Tag tone={tone}>{s.sample_status || "Awaiting Status"}</Tag>
                      {s.follow_up_date && (
            ...
            <ModalShell>
              ...
              <form onSubmit={handleUpdate}>
                <div className="mb-4">
                  ...
                  <select
                    ...
                    onChange={(e) => setForm((p) => ({ ...p, sample_status: e.target.value }))}
              ...
              <div className="mt-6 border-t border-slate-100 pt-5">
                ...
                <div className="flex flex-col gap-2">
                  {logs.map((log) => {
                    const tone = statusTone[log.sample_status] || defaultTone;
                    return (
                      <div
                        key={log.id}
                      ...
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <Tag tone={tone}>{log.sample_status}</Tag>
                          {log.follow_up_date && (
