              ...
              {hasStatus && (
                <div className="mx-3 mt-3 rounded-xl border border-slate-200 bg-white overflow-hidden">
                  <div className="flex items-center justify-between px-3 py-2 bg-slate-50 border-b border-slate-100">
                    <span className="flex items-center gap-1 text-[9px] font-bold uppercase tracking-widest text-slate-400"><Ic.Clipboard className="h-2.5 w-2.5"/>Current Status</span>
                    {currentFuDate && (
                    ...
                    <div className={cls("grid gap-0 border-b border-slate-50", currentResult && currentPriority ? "grid-cols-2" : "grid-cols-1")}>
                      ...
                      {currentPriority && (
                        <div className="px-3 py-2">
                          ...
                          <span className={cls("text-[10px] font-semibold leading-tight block", currentPriority === "High" ? "text-rose-600" : currentPriority === "Medium" ? "text-amber-600" : "text-slate-500")}>{currentPriority}</span>
                        </div>
                      )}
                    </div>
                  )}
              ...
              <div className="mx-3 mt-2 rounded-xl border border-slate-200 bg-white overflow-hidden">
                ...
                {(rfq.product_name || rfq.product_category) && (
                  <div className="px-3 pt-2 pb-1.5 border-b border-slate-50">
                    ...
                    {rfq.product_name && rfq.product_category && (
                      <p className="text-[10px] text-slate-400 mt-0.5 leading-tight">{rfq.product_category}{rfq.product_sub_category && <span className="text-slate-300"> · {rfq.product_sub_category}</span>}</p>
                    )}
                  </div>
                )}
                {(rfq.consumption_per_month || rfq.target_price) && (
                  <div className={cls("grid border-b border-slate-50", rfq.consumption_per_month && rfq.target_price ? "grid-cols-2" : "grid-cols-1")}>
                    {rfq.consumption_per_month && (
                      <div className={cls("px-3 py-2", rfq.target_price ? "border-r border-slate-50" : "")}>
                        ...
                        <p className="text-[11px] font-semibold text-slate-700">{rfq.consumption_per_month}{rfq.unit && <span className="text-[9px] font-normal text-slate-400 ml-0.5">{rfq.unit}</span>}</p>
                      </div>
                    )}
                    {rfq.target_price && (
                      <div className="px-3 py-2">
              ...
              <div className="mx-3 mt-2 rounded-xl border border-slate-200 overflow-hidden">
                <button type="button" onClick={toggleHistory}
                  ...
                  <span className="flex items-center gap-1.5 text-[9px] font-bold uppercase tracking-widest text-slate-400">
                    ...
                    {history !== null && <span className="ml-0.5 px-1.5 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[8px] font-bold normal-case tracking-normal">{history.length}</span>}
                  </span>
                  <div className={cls("transition-transform duration-200", historyOpen ? "rotate-180" : "")}><Ic.ChevD className="h-3 w-3 text-slate-400"/></div>
                </button>
                <AnimatePresence initial={false}>
                  {historyOpen && (
                            ...
                            <div key={log.id || i} className="px-3 py-2 space-y-1">
                                ...
                                <div className="shrink-0 flex items-center gap-1 text-right">
                                  {log.follow_up_date ? (
                                    <span className={cls("text-[9px] font-semibold whitespace-nowrap", dueCls(log.follow_up_date))}>
                                      {dueLabel(log.follow_up_date)}{log.follow_up_time && <span className="font-normal text-slate-400"> · {log.follow_up_time}</span>}
                                    </span>
                              ...
                              {(log.result || log.priority) && (
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  {log.priority && (
                                    <span className={cls("shrink-0 inline-flex items-center gap-0.5 text-[8px] font-bold", log.priority === "High" ? "text-rose-500" : log.priority === "Medium" ? "text-amber-500" : "text-slate-400")}>
                                      <span className={cls("h-1.5 w-1.5 rounded-full shrink-0", log.priority === "High" ? "bg-rose-400" : log.priority === "Medium" ? "bg-amber-400" : "bg-slate-300")}/>
                                      {log.priority}
                                    </span>
                                  )}
                                </div>
                    ...
                    {needsDescription && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.15 }} className="overflow-hidden">
                        <textarea value={description} onChange={e => { setDescription(e.target.value); setErrors(p => ({ ...p, description: undefined })); }}
                          ...
                          className={cls("w-full rounded-lg border px-2.5 py-2 text-[11px] text-slate-900 placeholder:text-slate-400 outline-none resize-none transition-all focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100", errors.description ? "border-rose-400 ring-2 ring-rose-100" : "border-slate-200")}/>
                    ...
                    {needsRejectReason && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.15 }} className="overflow-hidden">
                        <textarea value={rejectReason} onChange={e => { setRejectReason(e.target.value); setErrors(p => ({ ...p, rejectReason: undefined })); }}
                          ...
                          className={cls("w-full rounded-lg border px-2.5 py-2 text-[11px] text-slate-900 placeholder:text-slate-400 outline-none resize-none transition-all focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100", errors.rejectReason ? "border-rose-400 ring-2 ring-rose-100" : "border-slate-200")}/>
                      ...
                      {PRIORITY_OPTIONS.map(p => (
                        <button key={p} type="button"
                          ...
                          className={cls("flex-1 flex items-center justify-center gap-1 rounded-lg border py-1.5 text-[10px] font-bold transition-all active:scale-95",
                            priority === p ? cls(priorityColor[p]?.btn, "text-white shadow-sm") : "bg-white border-slate-200 text-slate-500 hover:bg-slate-50")}>
