// pages/repeatOrders/RepeatOrderUploadModal.jsx
...
export default function RepeatOrderUploadModal({ token, onClose, onDone }) {
  ...
  async function submit() {
    ...
    try {
      ...
      fd.append("file", file);
      const r = await fetch(`${API}/api/repeat-orders/upload`, {
        method: "POST",
  ...
  return (
    <Backdrop onClick={onClose}>
      <Sheet>
        <SheetHead title="Upload Sales Dump" subtitle="Sales Register Excel (.xlsx)" onClose={onClose} />
          ...
          <div className="space-y-4">
            ...
            <p className="text-[11px] leading-relaxed text-slate-400">
              <span className="block">
                Expected columns: Bill Date, Bill No, Party Name, City Name, Party GSTIN No, Mobile-1, Mobile-2,
                Product Name, Product Group Name, Product Category Name, Sales Man, Bill Amount.
              </span>
              <span className="mt-1 block">
                Rows are grouped by Party Name into one record per company. New companies land as{" "}
                <b>Unassigned</b> — nobody sees them until you or Communication assign them out. Re-uploading an
                overlapping report only adds bills not already on file and never touches assignment or follow-up
                status.
              </span>
            </p>
